...
double func_R0(double *qqq,double *par){
  double q1=qqq[0]*(xdirect-65.4023)/2./dist_det ;
  //double E1=pow(hbar*q1,2)/8./m_nc2nm;
  double qc=par[0];
  double ww=2.5E-03;//par[1];横橋修論
  double R0=par[1];
  ...
  double qc_up=0.217;//*(xdirect-65.4023)/dist_det; 横橋修論
...
*/
  ...
  for(Int_t i=0; i<num; i++){
    ...
    Double_t twopirad = 4*TMath::Pi();//*angle[i];
    ...
    if(i==2){
      ...
      for(Int_t i1=0; i1<nbin_q; i1++){
        ...
        //cout<<"AA_"<<AA[i1]<<"_BB_"<<BB[i1]<<"_q1_"<<i1<<endl;
    ...
    TLine *l1 = new TLine (xbegin,1e-3,xbegin, 83);
    TLine *l2 = new TLine (60.5,1e-3,60.5, 83);
    l1->SetLineColor(6);
    l2->SetLineColor(6);
    l1->Draw("ehsames");
    l2->Draw("ehsames");
    
    TLine *l3 = new TLine (xend,1e-3,xend, 83);
    TLine *l4 = new TLine (xcenter1,1e-3,xcenter1, 83);
    l3->SetLineColor(1);
    l4->SetLineColor(1);
    l3->Draw("ehsames");
    l4->Draw("ehsames");
    ...
    f0->SetParLimits(0,8.,9.);
    f0->FixParameter(1,1.);
    //f0->SetParLimits(2,0.,0.0000000001);
