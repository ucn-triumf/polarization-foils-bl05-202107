  ...
  double Rup;
    ...
    if(q1<qc){
      ...
      if(q1>=qc){
        ...
        double down_R=downrate*pow((q1-mm2*pow((q1*q1-qc*qc),0.5))/(q1+mm2*pow((q1*q1-qc*qc),0.5)),2);
        Rdown=down_R;//+down_R;
      }
    }
    R1=Rup+Rdown;
...
double func_R0_tan(double *qqq,double *par){
  ...
  //double q1=4*TMath::Pi()*(xdirect1-45.1074)/(2*dist_det1*lam1);
  //double q1=4*TMath::Pi()*(xdirect-60.4583)/(2*dist_det*lam1);//60.4583 65.4023
  double q1=4*TMath::Pi()*(xdirect-65.4023)/(2*dist_det*lam1);
  ...
  //double qc=4*TMath::Pi()*(xdirect-60.4583)/(2*dist_det*lmc);//par[0];
  double qc=4*TMath::Pi()*(xdirect-65.4023)/(2*dist_det*lmc);//par[0];
...
*/
  ...
  //angle[1] = TMath::Abs(60.4583 - xdirect)/dist_det; //rad
  //angle[2] = TMath::Abs(60.4583 - xdirect)/dist_det; //rad
  ...
  for(Int_t i=0; i<num; i++){
    ...
    tup[i]->SetAlias("toffo","(tof>11.3e3)*(tof)+(tof<11.3e3)*(tof+40.e3)"); // edited based on suggestion by KM on the August 3rd
    ...
    */
    ...
    f1->FixParameter(1,0.99);
    //f1->FixParameter(2,1.);
    f1->SetParLimits(2,0.,20.);
    //f1->SetParLimits(3,0.2,0.4);
