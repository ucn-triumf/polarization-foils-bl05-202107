...
Double_t xdirect    = 89.3295;//88.3551;
...
double func_R0(double *qqq,double *par){
  ...
  double Rup;
  double q13=0.13*0.8284697312*(89.3295-65.4023)/(89.3295-60.4583);
  double q145=0.145*0.8284697312*(89.3295-65.4023)/(89.3295-60.4583);
...
*/
  ...
  Double_t lambda_min  = 0.23;
  Double_t lambda_max  = 1.4;
  Int_t nbin_q  = 300;
  Double_t q_max  = 1.4;//0.6
  Int_t nrebinx = 1;
  Int_t nrebiny = 2;
  ...
  for(Int_t i=0; i<num; i++){
    ...
    Double_t lambda_coeff = 1.e-6*Conversion/(89.3295-65.4023)*(89.3295-60.4583)/Distance;// (89.3295-65.4023)/1439.(89.3295-60.4583)/1439.
    ...
    cout << kp[i]<<endl;
    hx[i] = new TH1F(Form("hx%d",i),Form("%s;X [mm];count/0.2mm",degstr[i].Data()),nbin,0.,range);
    ...
    hq0[i] = new TH1F(Form("hq0%d",i),Form("%s;Wavelength [nm];count/bin/25kp",degstr[i].Data()),nbin_q,0.,q_max);
    hq[i] = new TH1F(Form("hq%d",i),Form("%s;Wavelength [nm];count/bin/25kp",degstr[i].Data()),nbin_q,0.,q_max);
    ...
    */
    ...
    tup[i]->Draw(Form("toffo*%f>>hq4%d",lambda_coeff,i), thecut && cut_ref,"goff");
    //tup[i]->Draw(Form("%f/(toffo*%f)>>hq4%d",twopirad,
    //,i), thecut && cut_ref,"goff");
    ...
    if(i==2)hx[i]->Draw("eh");
    hx[i]->SetStats(0);
    hx[i]->GetYaxis()->SetTitleOffset(1.4);
    hx[i]->GetXaxis()->SetLabelSize(0.06);
    hx[i]->GetYaxis()->SetLabelSize(0.06);
    hx[i]->GetXaxis()->SetTitleSize(0.06);
    hx[i]->GetYaxis()->SetTitleSize(0.06);

    
    //else if(i!=6)hx[i]->Draw("ehsames");
    

    double Maxline=2.5e2*1.3;
    double Minline=1.2e-3;

    TLine *l1 = new TLine (xbegin,Minline,xbegin, Maxline);
    TLine *l2 = new TLine (xcenter,Minline,xcenter, Maxline);
    l1->SetLineColor(6);
    l2->SetLineColor(6);
    l1->Draw("ehsames");
    l2->Draw("ehsames");
    
    TLine *l3 = new TLine (xend,Minline,xend, Maxline);
    TLine *l4 = new TLine (xcenter1,Minline,xcenter1, Maxline);
    l3->SetLineColor(1);
    l4->SetLineColor(1);
    l3->Draw("ehsames");
    l4->Draw("ehsames");
    ...
    if(i==2){
      ...
      hq[i]->Fit(f1,"+","",0.26,1.26);
      hq[i]->Fit(f2,"+","",0.26,1.26);
      hq[i]->Fit(f3,"+","",0.26,1.26);
      hq[i]->SetStats(0);
      //hx[i]->SetStats(0);
      hq[i]->GetYaxis()->SetTitleOffset(1.4);
    ...
    hq[i]->GetYaxis()->SetRangeUser(0.,1.4);
    hq[i]->GetXaxis()->SetRangeUser(0.1,1.4);

    hq2[i]->GetYaxis()->SetRangeUser(0.,1.4);  
    hq2[i]->GetXaxis()->SetRangeUser(0.1,1.4);
