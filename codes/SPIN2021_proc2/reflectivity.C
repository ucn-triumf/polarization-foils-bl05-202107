...
double func_R0(double *qqq,double *par){
  ...
  double EN=sqrt(300/(pow(hbar,2)/8./m_nc2nm));
  cout<<"QQQ_"<<EN<<endl;
  ...
  if(E1>V_Fe_m){
      ...
      double kccc=sqrt(2.*m_nc2nm*(V_Fe+mu_n*in_mag))/hbar;
      qccc=2*kccc;
...
double func_R00(double *qqq,double *par){
  ...
  double Rup;
    ...
    if(E1>V_Fe_p){
      ...
      R1=funcdown;//(R1_nume/R1_deno)*Rdown+funcdown*(1.-Rdown);
      ...
      if(E1>V_Si){
        ...
        R1=funcdown;//1.*Rdown+funcdown*(1.-Rdown);
 ...
 */  
    ...
    TLine *l1 = new TLine (qccc,0,qccc,1.05);
