...
void DrawOneScan(TString rootfile_num ="20210716210153", Int_t iscan = 5) {
 ...
 c_tof->Update();
  ...
  std::cout << "min kp: " << tree->GetMinimum("kp") << std::endl;
  std::cout << "max kp: " << tree->GetMaximum("kp") << std::endl;
  
  TFile newfile(Form("../../data_scans/%s_list_%02d.root",rootfile_num.Data(), iscan),"RECREATE");
  // auto T = tree->CloneTree(0); // copy only the header
  // T->CopyEntries(tree);
  // newfile.Write();
