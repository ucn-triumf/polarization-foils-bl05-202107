...
*/
  ...
  */
  ...
  for(Int_t i=0; i<num; i++){
    ...
    hlambda2[i] = new TH1F(Form("hlambda2%d",i),Form("%s;Wavelength [nm];count/bin/25k",degstr2[i].Data()),nbin_lambda,0.,lambda_max);
    ...
    tup2[i]->Draw(Form("x*%f>>hx2%d",range,i), thecut,"goff");
    if(i==0) tup2[i]->Draw(Form("toffo*%f>>hlambda2%d",lambda_coeff,i), thecut && cut_dir,"goff");
    else tup2[i]->Draw(Form("toffo*%f>>hlambda2%d",lambda_coeff,i), thecut && cut_ref,"goff");
    ...
    //hx2[i]->Scale(25./kp2[i]);
    //hlambda2[i]->Scale(25./kp2[i]);
...
///*
    ...
    c1->cd(2);
    if(i!=0){
      hlambda[i]->SetStats(0);
    if(i==1)hlambda[i]->Draw("eh");
...
*/
  ...
  Double_t B[num-1] = {100,760,1000,500,300,760,760};
  // Double_t q_cuts[num_pol] = {0.2, 0.25, 0.3, 0.35, 0.4};
  ...
  for (Int_t i=0; i<num_pol; i++){
    ...
    gr[i]= new TGraphErrors(num-3, B, pol_at_qcut,0,error_pol_at_qcut);
