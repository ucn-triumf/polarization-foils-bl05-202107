...
Int_t M1_pol_check_ichi(){//AFP copy
  ...
  angle[1] = TMath::Abs(47.2685 - xdirect)/dist_det; 
  ...
  Int_t nbin = 512;
  Double_t range = 128.;
  Double_t range_x = 122.5;
  Double_t range_y = 118.6;
  ...
  */
  ...
  TCut cut_x = Form("x*%f>20 && x*%f<100",range_x,range_x);
  TCut cut_y = Form("y*%f>%f && y*%f<%f",range_y,ybegin,range_y,yend);
  TCut cut_y1 = Form("y*%f>%f && y*%f<%f",range_y,ybegin1,range_y,yend1);
  
  TCut cut_dir = Form("x*%f>%f && x*%f<%f",range_x,xcenter,range_x,xend);
  TCut cut_ref = Form("x*%f>%f && x*%f<%f",range_x,xbegin1,range_x,xcenter);
  TCut cut_ref1 = Form("x*%f>%f && x*%f<%f",range_x,xbegin1,range_x,xcenter1);
