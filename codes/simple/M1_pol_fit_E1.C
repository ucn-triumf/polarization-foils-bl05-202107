...
//Double_t xdirect    = 93.4728;92.4993
Double_t xdirect    =  92.4943;
Bool_t useMRfirst = 0; //use only MR events to avoid frame overlap
Bool_t useThinout = 0; //thinning out the event <1e4.
...
double func_R000(double *qqq,double *par){
    double q1=qqq[0];
  //double E1=pow(hbar*q1,2)/8./m_nc2nm;
  double qc=par[0];
  double ww=2.5E-03;//par[1];
...
TF1 *f000 = new TF1("",func_R000,0.01,0.5,3);
...
double func_R1(double *qqq,double *par){
  ...
  double R1;
    ...
    if(q1<qc){
      ...
      if(q1>qc){
        double up_R=0.5*R0*(1.-tanh((q1-qc)/ww))*(1.-alpha*(q1-qc));
        double down_R=downrate*R0*(1.-tanh((q1-qc)/ww))*(1.-alpha*(q1-qc));
        R1=up_R+down_R;
      }
    }
...
TF1 *f1 = new TF1("",func_R1,0.01,0.5,4);
...
TF1 *f2 = new TF1("",func_R2,0.01,0.5,4);
...
Int_t M1_pol_fit_E1(){
  ...
  namestr[1]="20210713225420_list.root";

  //namestr[2]="20210713215303_list.root";
  namestr[2]="20210713224531_list.root"; 

  namestr[3]="20210713225152_list.root"; //Fe 30 nm, theta = 0.69 deg., x = 0.0 mm, B = 1 mT from -8 mT  with AFP 760 mV
  namestr[4]="20210713225420_list.root"; //Fe 30 nm, theta = 0.69 deg., x = 0.0 mm, B = 1.5 mT from -8 mT  with AFP 760 mV
  namestr[5]="20210713222200_list.root";
...
*/
  ...
  angle[1] = TMath::Abs(52.5722 - xdirect)/dist_det; //rad
  angle[2] = TMath::Abs(68.4746 - xdirect)/dist_det; //rad
  angle[3] = TMath::Abs(68.4746 - xdirect)/dist_det; //rad
  angle[4] = TMath::Abs(68.4746 - xdirect)/dist_det; //rad
  angle[5] = TMath::Abs(68.4746 - xdirect)/dist_det; //rad
  angle[6] = TMath::Abs(68.4746 - xdirect)/dist_det; //rad
  ...
  degstr[1]=Form("_%frad",angle[1]);
  degstr[2]="M1_Direct";//Form("_%frad",angle[2]);
  degstr[3]=Form("_%frad",angle[3]);
  degstr[4]=Form("_%frad",angle[4]);
  degstr[5]=Form("_%frad",angle[5]);
  //degstr[1]=Form("267.2deg_%frad",angle[1]);//+namestr[1];
  //degstr[2]="266.92deg_";//+namestr[2];
  //degstr[3]="266.72deg_";//+namestr[3];
  //degstr[4]="266.9deg_";//+namestr[4];
  //degstr[5]="???_";//+namestr[5];
  degstr[6]=namestr[6];
  

  //  TLegend* leg = new TLegend(0.15, 0.75, 0.4, 0.98,"");
  TLegend* leg = new TLegend(0.75, 0.70, 1.00, 1.0,"");
  leg->SetFillColor(0);
  ...
  //Double_t xbegin=40.;
  //Double_t xcenter=80.;
  Double_t xbegin[6]={63,63.,63,63,63,63};
  Double_t xcenter[6]={73,73,73,73,73,73};
  Double_t xbegin11=45.;
  Double_t xcenter11=55.;
  Double_t xbegin22=63.;
  Double_t xcenter22=73.;
  Double_t xbegin33=70.;
  Double_t xcenter33=80.;
  Double_t xbegin44=60.;
  Double_t xcenter44=70.;
  Double_t xbegin55=60.;
  Double_t xcenter55=70.;
  //Double_t xbegin=65.;
  //Double_t xcenter=70.;
  ...
  for(Int_t i=0; i<num; i++){
    ...
    cut_ref[i] = Form("x*%f>%f && x*%f<%f",range,xbegin[i],range,xcenter[i]);
    TCut cut_ref1 = Form("x*%f>%f && x*%f<%f",range,xbegin22,range,xcenter22);
    TCut cut_ref2 = Form("x*%f>%f && x*%f<%f",range,xbegin2,range,xcenter2);
    //TCut cut_tof = Form("tof>1.0e3 && tof<39.9e3");
    //TCut cut_tof = Form("tof>36.376e3 && tof<36.835557e3");
    //TCut cut_tof = Form("tof>31.8e3 && tof<32.268e3");//6.95nm~7.05nm
    //TCut cut_tof = Form("tof>27.22487103e3 && tof<27.6824493e3");//6.95nm~7.05nm
    //0.03637660772
    //0.03683557184
    ...
    //hx3[i] = new TH1F(Form("hx3%d",i),Form("%s;X [mm];count/bin/25kp",degstr[i].Data()),nbin,0.,range);
    //hx2[i] = new TH1F(Form("hx%d",i),Form("%s;X [mm];count/bin/25kp",degstr[i].Data()),nbin,0.,range);
    ...
    else tup[i]->Draw(Form("toffo*%f>>hlambda%d",lambda_coeff,i), thecut && cut_ref1,"goff");

    tup[0]->Draw(Form("%f/(toffo*%f)>>hq0%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    tup[i]->Draw(Form("%f/(toffo*%f)>>hq%d",twopirad,lambda_coeff,i), thecut && cut_ref1,"goff");

    tup[i]->Draw(Form("toffo*%f:y*%f:x*%f>>hxylambda%d",lambda_coeff,range,range,i),cut_rpmt_basic && MRcut,"goff");

    
    if(i==0)leg->AddEntry(hx[i],degstr[i],"l");
    ...
    //hq3[i]->Add( hq[i], 1.);
    hq3[i]->Add(hq[i], hq2[i],1., 1.);

    //double error[i]=sqrt(pow(hq[i],2)/pow((hq[i]+hq2[i]),4)*hq2[i]+pow(hq2[i],2)/pow((hq[i]+hq2[i]),4)*hq[i]);
    ...
    if(i==2){
      ...
      for(Int_t i1=0; i1<nbin_q; i1++){
        AA[i1]=hq[i]->GetBinContent(i1);
        AAE[i1]=hq[i]->GetBinError(i1);
    ...
    TLine *l1[6];
    TLine *l2[6];
    l1[i] = new TLine (xbegin[i],1e-3,xbegin[i], 1e3);
    l2[i] = new TLine (xcenter[i],1e-3,xcenter[i], 1e3);
    l1[i]->SetLineColor(i+1);
    l2[i]->SetLineColor(i+1);
    if(i==3)l1[i]->Draw("eh");
    if(i==3)l2[i]->Draw("eh");
    ...
    //f0->SetParameter(0.,0.15);//m //第１引数が変数の番号、第２引数がその値
    f0->SetParLimits(0,0.11,0.15);
    f0->FixParameter(1,1.);
    //f0->SetParameter(0.,30.);//nm 
    //f0->SetParameter(1,0.05);
    //f0->SetParameter(2,1.);
    //f0->SetParLimits(1,0.95,1.00);
    //f0->SetParameter(3,0.1);
    //f0->SetParLimits(3,0.01,0.08);
    //f0->SetParameter(4,6.);
    ...
    f1->SetParameter(0.,0.15);//m //第１引数が変数の番号、第２引数がその値
    //f0->SetParameter(0.,30.);//nm 
    ...
    double alpha=par[3];
    */
    f000->SetParLimits(0,0.11,0.15);
   
    f000->SetParLimits(1,0.95,1.05);
    f000->SetParLimits(2,5.,10.);

    f000->SetNpx(1000);
    //f000->Draw("sames");

    //if(i==2) hq[i]->Fit(f000,"+","",0.1,0.47);
    ...
    */
    ...
    leg->Draw();
    hratio[i]->GetYaxis()->SetRangeUser(0.,1.2);
