...
Int_t M1_q_R_I_reflect2(){
  ...
  for(Int_t i=0; i<num; i++){
    ...
    //TTree *tree[i] = rootfile->Get<TTree>(tree_name);
    tree[i] = GetTree(namestr[i]);
    time[i] = (tree[i]->GetMaximum("kp")-tree[i]->GetMinimum("kp"))/25.;
    
    rootfile[i]  = namestr[i]; // name of the target root file
    TH1D *h_x = new TH1D("h_x","X histogram",nBinCut,x_cut_low,x_cut_up);
    tree[i]->Draw(Form("x*%f>>h_x", range), cut_xy * cut_rpmt* TCut(Form("%f",1./time)) , "EH"); //
