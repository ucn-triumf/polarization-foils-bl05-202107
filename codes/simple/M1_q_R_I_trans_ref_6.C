...
Int_t M1_q_R_I_trans_ref_6(){
  ...
  namestr[1]="20210713214225_list.root";

  //namestr[2]="20210713215303_list.root";
  namestr[2]="20210713215303_list.root"; 

  namestr[3]="20210714004440_list.root"; //Fe 30 nm, theta = 0.69 deg., x = 0.0 mm, B = 1 mT from -8 mT  with AFP 760 mV
  namestr[4]="20210713220438_list.root"; //Fe 30 nm, theta = 0.69 deg., x = 0.0 mm, B = 1.5 mT from -8 mT  with AFP 760 mV
...
*/
  ...
  angle[2] = TMath::Abs(66.3616 - xdirect)/dist_det; //rad
  angle[3] = TMath::Abs(73.7471 - xdirect)/dist_det; //rad
  angle[4] = TMath::Abs(68.1052 - xdirect)/dist_det; //rad
  angle[5] = TMath::Abs(68.1164 - xdirect)/dist_det; //rad
  ...
  degstr[1]=Form("267.2deg_%frad",angle[1]);
  degstr[2]=Form("266.92deg_%frad",angle[2]);
  degstr[3]=Form("266.72deg_%frad",angle[3]);
  degstr[4]=Form("266.9deg_%frad",angle[4]);
  ...
  //Double_t xbegin=40.;
  //Double_t xcenter=80.;
  Double_t xbegin[6]={0,45.,60,70,60,60};
  Double_t xcenter[6]={0,55,70,80,70,70};
  ...
  for(Int_t i=0; i<num; i++){
    ...
    TCut cut_tof = Form("tof>1.15e3 && tof<59.9e3");//6.95nm~7.05nm
    //0.03637660772
    //0.03683557184
