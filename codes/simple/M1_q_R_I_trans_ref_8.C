...
double func_R0(double *qqq,double *par){
  ...
  double R1;
    ...
    if(q1<qc){
      ...
      if(q1>qc){
        double up_R=uprate*R0*0.5*(1.-tanh((q1-mm*qc)/ww))*(1.-alpha*(q1-qc));
        //
        double down_R=downrate*R0*(1.-tanh((q1-qc)/ww));
...
TF1 *f0 = new TF1("",func_R0,0.01,0.5,5);
...
*/
  ...
  angle[2] = TMath::Abs(68.1052 - xdirect)/dist_det; //rad
  angle[3] = TMath::Abs(68.1052 - xdirect)/dist_det; //rad
  angle[4] = TMath::Abs(68.1052 - xdirect)/dist_det; //rad
  angle[5] = TMath::Abs(68.1052 - xdirect)/dist_det; //rad
  angle[6] = TMath::Abs(68.1052 - xdirect)/dist_det; //rad
  ...
  //Double_t xbegin=40.;
  //Double_t xcenter=80.;
  Double_t xbegin[6]={0,45.,63,63,63,63};
  Double_t xcenter[6]={0,55,73,73,73,73};
  Double_t xbegin11=45.;
  Double_t xcenter11=55.;
  Double_t xbegin22=60.;
  Double_t xcenter22=70.;
  ...
  for(Int_t i=0; i<num; i++){
    ...
    tup[i]->SetAlias("toffo","(tof>9.8e3)*(tof)+(tof<9.8e3)*(tof+40.e3)"); // edited based on suggestion by KM on the August 3rd
    ...
    else tup[i]->Draw(Form("toffo*%f>>hlambda%d",lambda_coeff,i), thecut && cut_ref[i],"goff");
    tup[0]->Draw(Form("%f/(toffo*%f)>>hq0%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    tup[i]->Draw(Form("%f/(toffo*%f)>>hq%d",twopirad,lambda_coeff,i), thecut && cut_ref[i],"goff");
    ...
    f0->SetParLimits(0,0.13,0.15);
    //f0->SetParameter(0.,30.);//nm 
    f0->SetParameter(1,0.05);
    //f0->SetParameter(2,1.);
    f0->SetParLimits(2,0.99,1.00);
    f0->SetParameter(3,0.1);
    f0->SetParameter(4,1.5);
