...
Double_t Distance = 18.101; //tentative
Double_t Conversion = 395.6;
Double_t dist_det   = 344.; //sample to detector [mm]
...
*/
  ...
  namestr[7]="20210714205602_list.root"; //760mV 276.2deg
  ...
  */
  ...
  angle[7] = TMath::Abs(58.1188- xdirect)/dist_det;
  ...
  for(Int_t i=0; i<num; i++){
    ...
    if(i==7){
      hpolratio[i]=(TH1F*)hq[i]->Clone(Form("hratio%d",i));
      hpolratio[i]->Add(hq[i-5], hq[i],1., -1.);
      hpolratio2[i]=(TH1F*)hq[i]->Clone(Form("hratio%d",i));
      hpolratio2[i]->Add(hq[i-5], hq[i],1., 1.);
    ...
    if(i==7)hpolratio[i]->SetLineColor(2);
    else {
