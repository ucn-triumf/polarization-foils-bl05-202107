...
*/
  ...
  Double_t pol_at_qcut211[num];
  Double_t error_pol_at_qcut211[num];
  ...
  for (Int_t i=0; i<num_pol; i++){
    ...
    for (Int_t j=0; j<num; j++){
      pol_at_qcut11[j] = hq211[j]->GetBinContent(ibin_pol[i]);
      error_pol_at_qcut11[j] = hq211[j]->GetBinError(ibin_pol[i]);

      cout <<   pol_at_qcut[j] << endl;
      // count << Form("At q=%.2f nm^{-1}, B=%.3f mT: Polarization power=", q_cuts[i], B[i])<<  pol_at_qcut[i] << endl;
    }

    gr[i]= new TGraphErrors(num, B11, pol_at_qcut,0,error_pol_at_qcut);
    gr[i]->GetXaxis()->SetRangeUser(0.2, 9);
    gr[i]->GetYaxis()->SetRangeUser(-1.2, 1.2);
    if (i==0) gr[i]->Draw("AP");
    else gr[i]->Draw("P");
    
    gr211[i]= new TGraphErrors(num, B211, pol_at_qcut11,0,error_pol_at_qcut11);
    gr211[i]->GetXaxis()->SetRangeUser(0.2, 9);
    gr211[i]->GetYaxis()->SetRangeUser(-1.2, 1.2);
