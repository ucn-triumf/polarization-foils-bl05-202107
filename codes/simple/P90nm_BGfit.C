...
double func_R0(double *qqq,double *par){
  ...
  double V_Fe_pp=V_Fe+mu_n*in_mag;//neV
  double V_Fe_mm=V_Fe-mu_n*in_mag;
...
double func_R3(double *qqq,double *par){
  double q1=qqq[0];
  double E1=pow(hbar*q1,2)/8./m_nc2nm;
  double d1=par[0];
  double in_mag=par[1];
  ...
  double downrate=0.8;
    ...
    if(E1>V_Fe_p){
      ...
      //double k1a=sqrt(2.*m_nc2*(E1-(V_Fe+mu_n*in_mag)))/hbar;//nm^-1
      double k1a=sqrt(2.*m_nc2*(E1-(V_Fe+mu_n*in_mag)))/hbar;//nm^-1
      double k1a2=sqrt(2.*m_nc2*(E1-(V_Fe+mu_n2*in_mag)))/hbar;//nm^-1
      double k2=sqrt(2.*m_nc2*(E1-V_Si))/hbar;//nm^-1

      double R1_nume=pow((k0*k1a-k1a*k2)*cos(k1a*d1),2)+pow((k0*k2-k1a*k1a)*sin(k1a*d1),2);
      double R1_deno=pow((k0*k1a+k1a*k2)*cos(k1a*d1),2)+pow((k1a*k1a+k0*k2)*sin(k1a*d1),2);

      double R2_nume=pow((k0*k1a2-k1a2*k2)*cos(k1a2*d1),2)+pow((k0*k2-k1a2*k1a2)*sin(k1a2*d1),2);
      double R2_deno=pow((k0*k1a2+k1a2*k2)*cos(k1a2*d1),2)+pow((k1a2*k1a2+k0*k2)*sin(k1a2*d1),2);

      R1=uprate*(R1_nume/R1_deno)+downrate*(R2_nume/R2_deno);//k1a*k1a-k0*k2 k0*k2-k1a*k1a
      ...
      if(E1>V_Si){
        ...
        //double k1a=sqrt(2.*m_nc2*(E1-(V_Fe+mu_n*in_mag)))/hbar;//nm^-1
        double alpha=sqrt(2.*m_nc2*((V_Fe+mu_n*in_mag)-E1))/hbar;//nm^-1
        double alpha2=sqrt(2.*m_nc2*((V_Fe+mu_n2*in_mag)-E1))/hbar;//nm^-1
        double k2=sqrt(2.*m_nc2*(E1-V_Si))/hbar;//nm^-1

        double R1_nume=pow((k0*alpha-alpha*k2)*cosh(alpha*d1),2)+pow((k0*k2+alpha*alpha)*sinh(alpha*d1),2);
        double R1_deno=pow((k0*alpha+alpha*k2)*cosh(alpha*d1),2)+pow((alpha*alpha-k0*k2)*sinh(alpha*d1),2);
        double R2_nume=pow((k0*alpha2-alpha2*k2)*cosh(alpha2*d1),2)+pow((k0*k2+alpha2*alpha2)*sinh(alpha2*d1),2);
        double R2_deno=pow((k0*alpha2+alpha2*k2)*cosh(alpha2*d1),2)+pow((alpha2*alpha2-k0*k2)*sinh(alpha2*d1),2);
  ...
  TF1 *f3 = new TF1("",func_R3,0.01,0.5,2);
 ...
 */  
    ...
    */
    ...
    f3->SetParameter(0.,90.e-9);//m //第１引数が変数の番号、第２引数がその値
    //f1->SetParameter(0.,30.);//nm 
    f3->SetParameter(1,2.);
    //f1->SetParLimits(1,1.8,1.999);
    f3->SetNpx(10000);
...
*/
  ...
  Double_t q_cuts11[num_pol] = {0.25, 0.3,0.225};
  ...
  // Double_t q_cuts[num_pol] = {0.2, 0.25, 0.3, 0.35, 0.4};
  Double_t pol_at_qcut11[num11-1];
  Double_t error_pol_at_qcut11[num11-1];

  Double_t ibin_pol11[num_pol];

  c1->cd(4); 

  ofstream ofs(path_R+Form("B_RdataOFF_qq225_%s.csv", scan_id.c_str()));  // ファイルパスを指定する
  ...
  for (Int_t i=0; i<num_pol; i++){
    ...
    for (Int_t j=1; j<num; j++){
      ...
      ofs << q_cut_i << "," << B[j-1] << ","<< pol_at_qcut[j] << ","<< error_pol_at_qcut[j] << endl;
