...
Int_t Pol_90nm_onoff(){
  ...
  const Int_t numm = 5;
  TString namestr11[numm];
  TString namestr211[numm];
  ...
  TString degstr11[num];
  TString degstr211[num];
  TString degstr_p11[num];
  ...
  Double_t angle11[num];
  Double_t angle211[num];
  for(Int_t i=0; i< num; i++){
  ...
  TLegend* leg11 = new TLegend(0.8, 0.5, 1.0, 1,"");
  TLegend* leg211 = new TLegend(0.8, 0.5, 1.0, 1,"");
  TLegend* leg311 = new TLegend(0.8, 0.5, 1.0, 1,"");
  ...
  */
  ...
  for(Int_t i=0; i<num; i++){
    ...
    tup011->Draw(Form("%f/(toffo*%f)>>hq011%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    hq011[i]->Scale(25./kp0);
    
    // Data with SF:ON
    tup11[i] = GetTree(namestr[i]);
    ...
    hq1[i] = new TH1F(Form("hq1%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    // tup[0]->Draw(Form("%f/(toffo*%f)>>hq0%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    tup11[i]->Draw(Form("%f/(toffo*%f)>>hq1%d",twopirad,lambda_coeff,i), thecut && cut_ref,"goff");
    ...
    leg->AddEntry(hq1[i],degstr11[i],"l");


  // Data with SF:OFF
    tup211[i] = GetTree(namestr211[i]);
    tup211[i]->SetAlias("toffo","(tof>10.e3)*(tof)+(tof<10.e3)*(tof+40.e3)"); // edited based on suggestion by KM on the August 3rd
    
    if(useMRfirst) kp211[i] = tup211[i]->GetMaximum("mp");
    else kp211[i] = (tup211[i]->GetMaximum("kp") - tup2[i]->GetMinimum("kp"));
    ...
    tup211[i]->Draw(Form("%f/(toffo*%f)>>hq211%d",twopirad2,lambda_coeff,i), thecut && cut_ref,"goff");
    hq211[i]->Scale(25./kp211[i]); 
    hq211[i]->Divide(hq011[i]); //Divide by the direct data
    hq211[i]->GetYaxis()->SetTitle("Reflectivity");
    hq211[i]->SetTitle("Reflectivity (SF OFF)");
    leg211->AddEntry(hq211[i],degstr211[i],"l");
    
    
    /*
    
    tup11[i] = GetTree(namestr11[i]);
    // tup[i]->SetAlias("toffo","(tof>9.e3)*(tof)+(tof<9.e3)*(tof+40.e3)");
    ...
    //hx[i] = new TH1F(Form("hx%d",i),Form("%s;X [mm];count/bin/25kp",degstr[i].Data()),nbin,0.,range);
    hlambda11[i] = new TH1F(Form("hlambda11%d",i),Form("%s;Wavelength [nm];count/bin/25k",degstr11[i].Data()),nbin_lambda,0.,lambda_max);
    hq011[i] = new TH1F(Form("hq011%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    hq11[i] = new TH1F(Form("hq11%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    hq0BG11[i] = new TH1F(Form("hq0BG11%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    hqBG11[i] = new TH1F(Form("hqBG11%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    
    // hxylambda[i] = new TH3F(Form("hxylambda%d",i),Form("%s;x [mm]; y [mm]; Wavelength [nm];count/bin/25kp",degstr[i].Data()), nbin/nrebinx,0.,range,nbin/nrebiny,0.,range,nbin_lambda,0.,lambda_max);
  
    //tup[i]->Draw(Form("x*%f>>hx%d",range,i), thecut,"goff");
    if(i==0) tup11[i]->Draw(Form("toffo*%f>>hlambda11%d",lambda_coeff,i), thecut && cut_dir,"goff");
    else tup11[i]->Draw(Form("toffo*%f>>hlambda11%d",lambda_coeff,i), thecut1 && cut_ref,"goff");
    tup11[0]->Draw(Form("%f/(toffo*%f)>>hq011%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    tup11[i]->Draw(Form("%f/(toffo*%f)>>hq11%d",twopirad,lambda_coeff,i), thecut && cut_ref,"goff");
    tup11[0]->Draw(Form("%f/(toffo*%f)>>hq0BG11%d",twopirad,lambda_coeff,i), thecut01 && cut_dir,"goff");
    tup11[i]->Draw(Form("%f/(toffo*%f)>>hqBG11%d",twopirad,lambda_coeff,i), thecut1 && cut_ref1,"goff");
    ...
    leg11->AddEntry(hq11[i],degstr11[i],"l");
    leg211->AddEntry(hq11[i],degstr11[i],"l");
    if(i!=0)leg311->AddEntry(hq11[i],degstr11[i],"l");
    //leg->AddEntry(hq2[i],degstr2[i],"l2");
    ...
    hlambda11[i]->Scale(25./kp11[i]);
    hq11[i]->Scale(25./kp11[i]);
    //hq011[i]->Scale(25./kp11[0]);
    hq011[i]->Scale(25./kp0);
    hqBG11[i]->Scale(25./kp11[i]);
    hq0BG11[i]->Scale(25./kp0);
    ...
    if(useMRfirst) kp211[i] = tup211[i]->GetMaximum("mp");
    else kp211[i] = tup211[i]->GetMaximum("kp");
    //cout << kp2[i]<<endl;
    hq0211[i] = new TH1F(Form("hq0211%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr211[i].Data()),nbin_q,q_min,q_max);
    hq211[i] = new TH1F(Form("hq211%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr211[i].Data()),nbin_q,q_min,q_max);
    hq02BG11[i] = new TH1F(Form("hq02BG11%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    hq2BG11[i] = new TH1F(Form("hq2BG11%d",i),Form("%s;q [nm^{-1}];count/bin/25kp",degstr11[i].Data()),nbin_q,q_min,q_max);
    
    tup211[0]->Draw(Form("%f/(toffo2*%f)>>hq0211%d",twopirad2,lambda_coeff,i), thecut0 && cut_dir,"goff");
    tup211[i]->Draw(Form("%f/(toffo2*%f)>>hq211%d",twopirad2,lambda_coeff,i), thecut && cut_ref,"goff");
    tup211[0]->Draw(Form("%f/(toffo*%f)>>hq02BG11%d",twopirad,lambda_coeff,i), thecut01 && cut_dir,"goff");
    tup211[i]->Draw(Form("%f/(toffo*%f)>>hq2BG11%d",twopirad,lambda_coeff,i), thecut1 && cut_ref1,"goff");

    hq211[i]->Add(hq211[i], hq2BG11[i],1., -1.);
    hq0211[i]->Add(hq0211[i], hq02BG11[i],1., -1.);
    hq211[i]->Scale(25./kp211[i]);
    hq0211[i]->Scale(25./kp211[0]);
    hq2BG11[i]->Scale(25./kp211[i]);
    hq02BG11[i]->Scale(25./kp211[0]);

    hq211[i]->Divide(hq0211[i]);
...
*/
    ...
    if(i==9){
      ...
      hq011[i]->SetLineColor(i+2);
      hq211[i]->SetLineColor(i+2);
      hqBG11[i]->SetLineColor(i+2);
      hq0BG11[i]->SetLineColor(i+2);
      hq02BG11[i]->SetLineColor(i+2);
      hq2BG11[i]->SetLineColor(i+2);
      ...
      hlambda11[i]->SetLineColor(i+1);
      //hratio[i]->SetLineColor(i+1);
      hq11[i]->SetLineColor(i+1);
      hq011[i]->SetLineColor(i+1);
      hq211[i]->SetLineColor(i+1);
      //hpolratio[i]->SetLineColor(i+1);
      //hpolratio2[i]->SetLineColor(i+1);
      hqBG11[i]->SetLineColor(i+1);
      hq0BG11[i]->SetLineColor(i+1);
      hq02BG11[i]->SetLineColor(i+1);
      hq2BG11[i]->SetLineColor(i+1);
    }
