...
const string scan_id = "30nm_scan_long_1";
const string run_id="20210716220736";
const Int_t num = 2; // this should be the half of the number of the files obtained by the scan 
...
Int_t Pol_Power_30nm_scan_long_1(){
  ...
  for(Int_t i=0; i< num; i++){
    angle[i]=TMath::Abs(47.0239 - xdirect)/dist_det;
    angle2[i]=TMath::Abs(47.0239 - xdirect)/dist_det;
    angledeg[i]=angle[i]*180./TMath::Pi()/2.;
    angledeg2[i]=angle2[i]*180./TMath::Pi()/2.;
  ...
  // Direct data
    ...
    hq0[i] = new TH1F(Form("hq0_%d",i),Form("%s;q [nm^{-1}];count/bin/25kp","Direct"),nbin_q,q_min,q_max);
    tup0->Draw(Form("%f/(toffo*%f)>>hq0_%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    hq0[i]->Scale(25./kp0);
    ...
    hq[i]->Scale(25./kp[i]);
    hq[i]->Divide(hq0[i]); //Divide by the direct data
    hq[i]->GetYaxis()->SetTitle("Reflectivity");
    hq[i]->SetTitle("Reflectivity (SF ON)");
...
// Calculate Polarization Power
  ...
  hpolratio[i]->Add(hq[i], hq2[i],-1., 1.); // hq: ON, hq2: OFF, calculate Noff - Non
