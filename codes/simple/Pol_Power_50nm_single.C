 ...
 */  
  ...
  const Int_t num_pol = 3;
    ...
    Double_t B[num-2] = {8.13};
    // Double_t q_cuts[num_pol] = {0.2, 0.25, 0.3, 0.35, 0.4};
    Double_t pol_at_qcut[num-2];
    Double_t error_pol_at_qcut[num-2];
    ...
    for (Int_t i=0; i<num_pol; i++){
      ...
      gr[i]= new TGraphErrors(num-2, B, pol_at_qcut,0,error_pol_at_qcut);
