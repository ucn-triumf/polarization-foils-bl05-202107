...
int all_B_90nm() {
    ...
    //ifstream fcsv("results/B_scan_1966.csv");
    ifstream fcsv("results/B_scan_q225.csv");
  ...
  for(int i=0;i<num;i++){
    cout << vec_q[i]<<"_"<< vec_I[i]<<"_"<< vec_H[i] <<"_" << vec_H_E[i]<< endl;
    //double *q1[i]=&vec_q[i];
    if(vec_q[i]<0.221){
      q1[i]=vec_q[i];
      I1[i]=vec_I[i];
  ...
  for(int i=0;i<num;i++){
    if(vec_q[i]==0.246667){
        ...
        gr1->GetYaxis()->SetRangeUser(0., 1.3);
        //gr1->GetXaxis()->SetRange(0,9);
        gr1->GetXaxis()->SetRangeUser(0.,9.);
        gr1->GetYaxis()->SetTitle("Reflectance");
        gr1->SetTitle("");
        gr1->Draw("AP");
