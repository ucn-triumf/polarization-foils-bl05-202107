...
Int_t overview(){
  ...
  TString namestr[num];
  namestr[0]="20210714184125_list.root"; //M1 reflect (direct) 1hour
  namestr[1]="20210714193654_list.root";
  namestr[2]="20210717040703_list.root";
  ...
  */ 
  ...
  TString degstr[num];
  degstr[0]="Direct(20210714184125)";
  degstr[1]="Direct(20210714193654)";
  degstr[2]="Direct(20210717040703)";
...
*/
  angle[0] = TMath::Abs(70.5 - xdirect)/dist_det; //rad
  angle[1] = TMath::Abs(70.5 - xdirect)/dist_det; //rad
  angle[2] = TMath::Abs(70.5 - xdirect)/dist_det; //rad
  ...
  for(Int_t i=0; i<num-4; i++){
    ...
    if(i==1) tup[i]->Draw(Form("toffo*%f>>hlambda%d",lambda_coeff,1), thecut && cut_dir,"goff");
    if(i==2) tup[i]->Draw(Form("toffo*%f>>hlambda%d",lambda_coeff,2), thecut && cut_dir,"goff");
    ...
    //tup[1]->Draw(Form("%f/(toffo*%f)>>hq0%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
    //tup[2]->Draw(Form("%f/(toffo*%f)>>hq0%d",twopirad,lambda_coeff,i), thecut0 && cut_dir,"goff");
...
/*
    ...
    leg->Draw();
*/
    hratio[i]->GetYaxis()->SetRangeUser(1.e-2,2.);
    //hq[i]->GetYaxis()->SetRangeUser(0.1,0.9);
    hq[i]->GetYaxis()->SetRangeUser(1.e-2,2.);
    hq[i]->GetXaxis()->SetRangeUser(0.1,0.9);

  }

  hratio[1]->GetYaxis()->SetRangeUser(1.e-2,2.);
  hq[1]->GetYaxis()->SetRangeUser(1.e-2,2.);
  ...
  c1->SaveAs(path_R+"overview.png");
  c1->SaveAs(path_R+"overview.root");
