...
Int_t q_R_I_on2(){
  ...
  namestr[2]="20210715080233_list.root"; 
  namestr[3]="20210715082018_list.root";
  namestr[4]="20210715085144_list.root"; //Fe 30 nm, theta = 0.69 deg., x = 0.0 mm, B = 1 mT from -8 mT  with AFP 760 mV
  namestr[5]="20210715085714_list.root"; //Fe 30 nm, theta = 0.69 deg., x = 0.0 mm, B = 1.5 mT from -8 mT  with AFP 760 mV
  namestr[6]="20210715083141_list.root"; 
  namestr[7]="20210715084052_list.root";
  */
  namestr[1]="20210715073913_list.root";
...
*/
  ...
  for(Int_t i=0; i<num; i++){
    ...
    hqq[i]=(TH1F*)hq[i]->Clone(Form("hqq%d",i));
    ...
    if(i==9){
      ...
      hqq[i]->SetLineColor(i+1);
      hq0[i]->SetLineColor(i+1);
    }

    c1->cd(1);
    if(i==0)hq0[i]->Draw("eh");
    else hq0[i]->Draw("ehsames");
    leg->Draw();

    c1->cd(2);
    if(i==0)hqq[i]->Draw("eh");
    else hqq[i]->Draw("ehsames");
    leg->Draw();
  ...
  hqq[i]->GetYaxis()->SetRangeUser(0.,1.);
  ...
  c1->SaveAs(path_R+"q_R_I_on2.png");
  c1->SaveAs(path_R+"q_R_I_on2.root");
