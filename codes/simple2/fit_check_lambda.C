...
double func_R0(double *qqq,double *par){
  double lam1=qqq[0];
  double q1=4*TMath::Pi()*(xdirect-45.1074)/(2*dist_det*lam1);//upの角度を用いた
...
Int_t fit_check_lambda(){
  ...
  */
  ...
  for(Int_t i=0; i<num; i++){
    ...
    if(i==0) tup[i]->Draw(Form("toffo*%f>>hq0%d",lambda_coeff,i), thecut && cut_dir,"goff");
    else tup[i]->Draw(Form("toffo*%f>>hq%d",lambda_coeff,i), thecut1 && cut_ref,"goff");
