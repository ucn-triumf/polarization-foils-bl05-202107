...
Int_t FeMirror3(){
  ...
  // namestr[7]="20210715083711_list.root"; //Fe 30 nm, theta = 0.69 deg. x = 0.0 mm , B = 2 mT  -> 3 mT
  namestr[1]="20210716210153=list.root";
  //  namestr[1]="20210716201056_list.root"; //Fe 30 nm, theta = 0.69 deg., B = 1.00-1.48 mT from -8 mT
  ...
  //
  degstr[1]="B = 1.00-1.48 mT from -8 mT";
  ...
  Int_t nrebinx = 1;
  Int_t nrebiny = 2;
  Int_t kp_start = 3062552;
  Int_t kp_end = 3070053;
  Int_t LLD  = 500.;
  //  Double_t HLD  = 7400.;
  ...
  TCut thecut0;
  TCut kpcut = Form("kp>%i && kp<%i", kp_start, kp_end);
  ...
  for(Int_t i=0; i<num; i++){
    ...
    if(i==0) tup[i]->Draw(Form("toffo*%f>>hlambda%d",lambda_coeff,i), thecut && cut_dir&& kpcut,"goff");
    else tup[i]->Draw(Form("toffo*%f>>hlambda%d",lambda_coeff,i), thecut && cut_ref && kpcut,"goff");
    tup[0]->Draw(Form("%f/(toffo*%f)>>hq0%d",twopirad,lambda_coeff,i), thecut0 && cut_ref && kpcut,"goff");
    tup[i]->Draw(Form("%f/(toffo*%f)>>hq%d",twopirad,lambda_coeff,i), thecut && cut_ref && kpcut,"goff");
    tup[i]->Draw(Form("toffo*%f:y*%f:x*%f>>hxylambda%d",lambda_coeff,range,range,i),cut_rpmt_basic && MRcut && kpcut,"goff");
  ...
  c1->SaveAs(Form("FeMirror3.png"));
  c1->SaveAs(Form("FeMirror3.root"));

#if 1
  TFile *outfile = TFile::Open("FeMirrorhist3.root","RECREATE");
  for(Int_t i=0; i<num; i++){
    hx[i]->Write();
    hlambda[i]->Write();
    hratio[i]->Write();
