#include "MakeNikiFRP.h"
#include "DrawNikiFRP.cxx"
...
int main(int argc, char *argv[])
{
  ...
  MakeNikiFRP(DataFileName); //for FRP
  if(nOption<2 && drawflag)  DrawNikiFRP(DataFileName); //for FRP
...
Int_t *MakeNikiFRP(char* DataFileName)
{
  Int_t i=0,nlines=0;
  ...
  while(1){
    ...
    //if time is later than sett+dt, then Fill data in tuple
      ...
      if((sRpmt.iPulse[0]+sRpmt.iPulse[1])!=0)  RPMT_X = Double_t(sRpmt.iPulse[1]+sRpmt.iPulse[3])/Double_t(sRpmt.iPulse[0]+sRpmt.iPulse[1]+sRpmt.iPulse[2]+sRpmt.iPulse[3]);
      else  RPMT_X = -1.;
      if((sRpmt.iPulse[2]+sRpmt.iPulse[3])!=0)  RPMT_Y = Double_t(sRpmt.iPulse[2]+sRpmt.iPulse[3])/Double_t(sRpmt.iPulse[0]+sRpmt.iPulse[1]+sRpmt.iPulse[2]+sRpmt.iPulse[3]);
      else  RPMT_Y = -1.;
